#!/bin/bash
# Author: fansik
# Description: The script is used for init CentOS7.x
# Date: 2018-12-4
...
set_system(){
    ...
    yum -y install chrony expect libevent libevent-devel openssh-clients openssh openssl-devel dmidecode gmp perl-DBI vim-common vim-enhanced vim-minimal iotop dstat wget make wget lrzsz tree sudo traceroute screen nc bc lsof sysstat control-center gcc gcc-c++ glibc glibc-common glibc-devel libaio libgcc libstdc++ libstdc++-devel libXp git  bind-utils net-tools epel-release.noarc
...
set_user(){
    groupadd ops
    useradd -g ops fansik
    echo "fanjinbao"|passwd fansik --stdin
    echo "fanjinbao_%@!" |passwd root --stdin
    echo """%ops  ALL=(root)       /bin/su -
            %ops  ALL=(ALL)       /bin/su - fansik,NOPASSWD: ALL
    """ >> /etc/sudoers
}
